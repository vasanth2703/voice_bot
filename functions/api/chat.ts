...
export async function onRequestPost(context: {
  ...
  try {
    ...
    if (!apiKey) {
      return new Response(
        JSON.stringify({
          reply: "I'd love to answer that, but my connection is currently missing its API key configuration. Please make sure the GEMINI_API_KEY environment variable is configured in the Cloudflare Pages dashboard.",
        }),
        { status: 200, headers }
      );
    }
